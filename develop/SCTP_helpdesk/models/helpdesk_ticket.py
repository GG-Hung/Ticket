...
class HelpdeskTicket(models.Model):
    ...
    def _compute_team_id(self):
        for ticket in self:
            if self.env.company.SCTP_helpdeskdefault_team_id:
                ticket.team_id = self.env.company.SCTP_helpdeskdefault_team_id
    ...
    user_id = fields.Many2one(
        ...
        store=True,
        readonly=False,
        domain="team_id and [('id', 'in', user_ids)] or []",  # noqa: B950,E501
    )
    ...
    proccesing = fields.Boolean(related="stage_id.proccesing", store=True)
    ...
    def _creation_subtype(self):
        return self.env.ref("SCTP_helpdesk.hlp_tck_created")
        ...
        for vals in vals_list:
            ...
            if self.env.context.get("fetchmail_cron_running") and not vals.get(
                ...
                channel_email_id = self.env.ref(
                    "SCTP_helpdesk.helpdesk_ticket_channel_email",
