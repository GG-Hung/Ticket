...
class HelpdeskTeam(models.Model):
    ...
    todo_ticket_count_proccesing = fields.Integer(
        string="Number of tickets proccesing", compute="_compute_todo_tickets"
    )
    todo_ticket_count_closed = fields.Integer(
        string="Number of tickets closed", compute="_compute_todo_tickets"
    )
    ...
    def _compute_todo_tickets(self):
        ...
        fetch_data = ticket_model.read_group(
            [("team_id", "in", self.ids)],
            ["team_id", "user_id", "unattended", "proccesing", "closed", "priority"],
            ["team_id", "user_id", "unattended", "proccesing", "closed", "priority"],
            lazy=False,
        )
        result = [
        ...
        print('5425125215125',fetch_data)
        for team in self:
            team.todo_ticket_count = sum(r[6] for r in result if r[0] == team.id)
            team.todo_ticket_count_unassigned = sum(
                r[6] for r in result if r[0] == team.id and not r[1]
            )
            team.todo_ticket_count_unattended = sum(
                r[6] for r in result if r[0] == team.id and r[2]
            )
            team.todo_ticket_count_proccesing = sum(
                r[6] for r in result if r[0] == team.id and r[3]
            )
            team.todo_ticket_count_closed = sum(
                r[6] for r in result if r[0] == team.id and r[4]
            )
            team.todo_ticket_count_high_priority = sum(
                r[6] for r in result if r[0] == team.id and r[5] == "3"
            )

    def _alias_get_creation_values(self):
        values = super()._alias_get_creation_values()
        values["alias_model_id"] = self.env.ref(
            "SCTP_helpdesk.model_helpdesk_ticket"
        ).id
